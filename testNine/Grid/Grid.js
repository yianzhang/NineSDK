//jQuery
document.writeln("<script src='https://ajax.googleapis.com/ajax/libs/jquery/2.0.0/jquery.min.js'></script>");

//Constant
var LIMIT_IN_SAME_FACE = Math.PI/6;
var LIMIT_IN_SAME_POLYLINE = Math.PI*5/6;
var LIMIT_ERROR = 1e-6;

//AllBody
document.writeln("<script src='Grid/AllBody.js'></script>");

//BodySet
document.writeln("<script src='Grid/Body.js'></script>");
document.writeln("<script src='Grid/BodySet.js'></script>");

//FaceSet
document.writeln("<script src='Grid/Face.js'></script>");
document.writeln("<script src='Grid/FaceSet.js'></script>");

//TriangleSet
document.writeln("<script src='Grid/Triangle.js'></script>");
document.writeln("<script src='Grid/TriangleSet.js'></script>");

//Polyline
document.writeln("<script src='Grid/Line.js'></script>");
document.writeln("<script src='Grid/Polyline.js'></script>");

//Vector
document.writeln("<script src='Grid/Vector.js'></script>");

//Point
document.writeln("<script src='Grid/Point.js'></script>");

//Reader
document.writeln("<script src='Grid/Reader.js'></script>");
